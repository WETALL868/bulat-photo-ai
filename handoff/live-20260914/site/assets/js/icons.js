/* Иконки интерфейса: один общий словарь, значение каждого ключа — содержимое
   <svg viewBox="0 0 24 24">. Это служебная графика витрины, нарисованная
   линиями под один стиль.

   Фирменного знака MAX здесь нет и быть не должно: это чужой товарный знак,
   он лежит официальным файлом assets/img/max-icon.svg и подключается
   картинкой. Путь к нему приходит из data/site.json (messengers.max.icon). */
window.HB_ICONS = {
